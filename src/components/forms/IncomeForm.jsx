...
function normalizeCategory(value) {
  ...
  if (lower === "profit" || lower === "investment") {
    return "investment";
  }
...
function IncomeForm({
  ...
  const [formData, setFormData] = useState(() => ({
    ...
    receivedDate: formatDateForInput(
      initialData?.receivedDate || initialData?.date
    ),
  ...
  const validate = (data = formData) => {
    ...
    if (!data.name || !data.name.trim()) {
      validationErrors.name =
        "اسم مصدر الدخل مطلوب (Name is required)";
    } else if (data.name.trim().length > 100) {
      validationErrors.name =
        "اسم مصدر الدخل لا يمكن أن يتجاوز 100 حرف";
    }

    if (!data.type) {
      validationErrors.type =
        "نوع الدخل مطلوب (Type is required)";
    }
    ...
    if (!data.receivedDate) {
      validationErrors.receivedDate =
        "تاريخ الاستلام مطلوب (Received Date is required)";
    }

    if (
      !["once", "weekly", "monthly", "yearly"].includes(
        data.recurrence
      )
    ) {
      validationErrors.recurrence =
        "يرجى اختيار نوع التكرار";
    }
  ...
  return (
    <div className="relative mx-auto max-w-2xl overflow-hidden rounded-3xl border border-slate-200/90 bg-white shadow-xl shadow-slate-100 transition-all dark:border-slate-800 dark:bg-slate-900 dark:shadow-none">
      {/* Top accent */}
      <div
      ...
      <div className="p-6 sm:p-10">
        {/* Header */}
        <div className="mb-8 flex flex-col items-start justify-between gap-4 border-b border-slate-100 pb-6 dark:border-slate-800 sm:flex-row sm:items-center">
          <div className="flex items-center gap-4">
            <div
              className={`flex h-13 w-13 items-center justify-center rounded-2xl shadow-inner ring-1 ${
                isEdit
                  ? "bg-amber-50 text-amber-600 ring-amber-200/80 dark:bg-amber-950/40 dark:text-amber-400 dark:ring-amber-800/60"
        ...
        {/* Server error */}
        {serverError && (
          <div className="mb-6 rounded-2xl border border-red-200 bg-red-50/90 p-4 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/30 dark:text-red-300">
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              maxLength={100}
              className={`w-full rounded-xl border bg-slate-50/50 px-4 py-3 text-sm text-slate-800 outline-none transition-all focus:bg-white focus:ring-4 dark:bg-slate-800/80 dark:text-slate-100 dark:focus:bg-slate-800 ${
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="0.00"
                  className={`w-full rounded-xl border bg-slate-50/50 py-3 pr-4 pl-16 text-sm font-semibold text-slate-800 outline-none transition-all focus:bg-white focus:ring-4 dark:bg-slate-800/80 dark:text-slate-100 ${
                    errors.amount &&
                    touched.amount
                      ? "border-rose-400 focus:border-rose-500 focus:ring-rose-500/10 dark:border-rose-700"
                      : "border-slate-300/80 focus:border-emerald-500 focus:ring-emerald-500/10 dark:border-slate-700"
                ...
                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4">
                  <span className="rounded-md bg-slate-200/80 px-2 py-0.5 text-xs font-bold text-slate-700 dark:bg-slate-700 dark:text-slate-300">
                    {formData.currency ||
                      "EGP"}
                  </span>
                </div>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div>
              ...
              <input
                ...
                maxLength={10}
                className={`w-full rounded-xl border bg-slate-50/50 px-4 py-3 text-sm font-semibold uppercase tracking-wider text-slate-800 outline-none transition-all focus:bg-white focus:ring-4 dark:bg-slate-800/80 dark:text-slate-100 ${
                  errors.currency &&
                  touched.currency
                    ? "border-rose-400 focus:border-rose-500 focus:ring-rose-500/10 dark:border-rose-700"
                    : "border-slate-300/80 focus:border-emerald-500 focus:ring-emerald-500/10 dark:border-slate-700"
                ...
                (option) => (
                  <label
                    ...
                    className={`cursor-pointer rounded-xl border px-3 py-3 text-center transition-all ${
                      formData.recurrence ===
                      option.value
                        ? "border-emerald-500 bg-emerald-50 text-emerald-700 ring-2 ring-emerald-500/10 dark:bg-emerald-950/40 dark:text-emerald-300"
                        : "border-slate-300 bg-slate-50/50 text-slate-600 hover:border-slate-400 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800/80 dark:text-slate-300"
                    }`}
                  >
                    <input
                      type="radio"
          ...
          <div>
            ...
            <textarea
              ...
              maxLength={500}
              className={`w-full rounded-xl border bg-slate-50/50 p-4 text-sm text-slate-800 outline-none transition-all focus:bg-white focus:ring-4 dark:bg-slate-800/80 dark:text-slate-100 ${
          ...
          {/* Actions */}
          <div className="flex flex-col-reverse gap-3.5 border-t border-slate-100 pt-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-end">
            {onCancel && (
              <button
                type="button"
                onClick={onCancel}
                disabled={isSubmitting}
                className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition-all hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
              >
                إلغاء وتراجع
              </button>
            )}
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className={`inline-flex items-center justify-center gap-2 rounded-xl px-8 py-3 text-sm font-semibold text-white shadow-md transition-all duration-200 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60 ${
                isEdit
                  ? "bg-linear-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500"
                  : "bg-linear-to-r from-emerald-600 via-teal-600 to-emerald-600 hover:from-emerald-500 hover:to-teal-500"
              }`}
            >
              {isSubmitting ? (
                <>
